...
function usernameToHash($s) {
		...
		$s1 = trim($s1);
		if (strlen($s1) > 12) $s1 = substring($s1, 0, 12);
		$l = 0;
...
function combatCalc($attack, $defense, $strength, $hits, $ranged, $magic, $prayer)$calcatkdefstr = (($attack + $defense + $strength + $hits) * .25);
...
function drawCharacter($hc, $hsprite, $sc, $tc, $gender, $pc){
		...
		$gender = ($gender == 1) ? 0 : 1;
		return '<img src="/elite/css/images/character/heads/c'.$hc.'t'.$hsprite.'sc'.$sc.'.png" id="head" alt="head" />
					 <img src="/elite/css/images/character/bodies/c'.$tc.'sc'.$sc.'t'.$gender.'.png" id="body" alt="body" />
					 <img src="/elite/css/images/character/legs/c'.$pc.'.png" id="leg" alt="leg" />';
}
?>
